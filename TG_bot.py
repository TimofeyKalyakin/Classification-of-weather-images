...
# Маппинг индексов классов на тип погоды
IDX_TO_CLASS = {
    0: 'dew', 1: 'fog/smog', 2: 'frost', 3: 'glaze', 4: 'hail',
    5: 'lightning', 6: 'rain', 7: 'rainbow', 8: 'rime',
    9: 'sandstorm', 10: 'snow'
}

# Загрузка модели
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = create_model('vgg16', pretrained=False, num_classes=11)
model.load_state_dict(torch.load('vgg16_weather_classifier.pth'))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        'Привет! Отправь мне изображение погоды, и я попробую её определить.'
    )
